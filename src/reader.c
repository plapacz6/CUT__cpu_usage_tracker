 ...
 *
 * @param buff_M buffer with content of /proc/stat
 * @param buff_M_size size of buff in 1sth param
 * @param msg_array
 * @param cpuN number of cpu cores, and size of array in 4th param
 * @param size_msg1core
 */
void read_One_set(const char buff_M[], size_t /*buff_M_size*/, char* msg_array, size_t cpuN, size_t size_msg1core) {

    const char *begin1 = buff_M;
    const char *begin2 = NULL;
    begin1 = &buff_M[0];
    size_t core = 0;
    // printf("\n cpuN: %lu\n", cpuN);
    // buff_M[buff_M_size - 1] = '\0';
